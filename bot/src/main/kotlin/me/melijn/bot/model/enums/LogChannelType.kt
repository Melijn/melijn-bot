...
enum class LogChannelType(val text: String = this.toString(), val parentNodes: Array<String> = emptyArray()) : InferredChoiceEnum {
    ...
    BIRTHDAY("Birthday", arrayOf()),
    PUNISHMENT_POINTS("PunishmentPoints", arrayOf("punishment", "punishments", "pp", "pplc")),

    JOIN("Join", arrayOf()),
    LEAVE("Leave", arrayOf()),
    KICKED("Kicked", arrayOf()),
    BANNED("Banned", arrayOf()),
    PRE_VERIFICATION_JOIN("PreVerificationJoin", arrayOf("pvj")),
    PRE_VERIFICATION_LEAVE("PreVerificationLeave", arrayOf("pvl"));
